...
game:GetService('UserInputService').InputBegan:connect(function(key, gpe)
           if (not gpe) then
               if key.KeyCode == Enum.KeyCode.RightControl then
                   uiui.toggled = not uiui.toggled;
                   for uiui, data in next, uiui.queue do
                       local pos = (uiui.toggled and data.p or UDim2.new(-1, 0, -0.5,0))
                       data.w:TweenPosition(pos, (uiui.toggled and 'Out' or 'In'), 'Quad', 0.15, true)
                       wait();
                   end
               end
           end
