...
game:GetService('UserInputService').InputBegan:connect(function(key, gpe)
           if (not gpe) then
               if key.KeyCode == Enum.KeyCode.RightControl then
                   uiui.disabled = not uiui.enabled;
                   for uiui, data in next, uiui.queue do
                       local pos = (uiui.disabled and data.p or UDim2.new(-1, 0, -0.5,0))
                       data.w:TweenPosition(pos, (uiui.enabled and 'Out' or 'In'), 'Quad', 0.15, true)
                       wait();
                   end
               end
           end
